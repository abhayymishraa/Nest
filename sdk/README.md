...
[`NestAPIError`](./src/nest_api/errors/nestapierror.py) is the base class for all HTTP error responses. It has the following properties:
...
**Inherit from [`NestAPIError`](./src/nest_api/errors/nestapierror.py)**:
* [`ResponseValidationError`](./src/nest_api/errors/responsevalidationerror.py): Type mismatch between the response data and the expected Pydantic model. Provides access to the Pydantic validation error via the `cause` attribute.
